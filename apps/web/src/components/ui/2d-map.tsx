...
function StyleModifier() {
  ...
  useEffect(() => {
    ...
    // Modify land to dark grey
    if (map.getLayer("land")) {
      map.setPaintProperty("land", "fill-color", "#333333");
    }
...
export default function WorldMapWithFloats({ setIs3D }: { setIs3D: (is3D: boolean) => void }) {
  ...
  return (
    <div className="relative w-full h-full">
      <Map className="h-full w-full">
        ...
        {floats.map((float) => (
          <MapMarker key={float.id} latitude={float.lat} longitude={float.lng}>
            <MarkerContent>
              <div className="w-4 h-4 bg-blue-500 rounded-full" />
            </MarkerContent>
            <MarkerPopup>
              <div>Float ID: {float.id}</div>
              <div>Lat: {float.lat.toFixed(2)}</div>
              <div>Lng: {float.lng.toFixed(2)}</div>
            </MarkerPopup>
          </MapMarker>
        ))}
        <StyleModifier />
